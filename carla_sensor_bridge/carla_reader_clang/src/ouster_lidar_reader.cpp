...
OusterLidarReader::OusterLidarReader(
    carla::SharedPtr<carla::client::Actor> parent,
  ...
  carla::geom::Transform transform(carla::geom::Location(0.85f, 0.0f, 1.10f),
  ...
  if (sensor_) {
    ...
    std::cout << "[CARLA Reader] Spawned Ouster LiDAR (ID: " << sensor_->GetId()
              << ")\n";
  } else {
...
OusterLidarReader::~OusterLidarReader() { destroy(); }
...
void OusterLidarReader::onSensorData(
    carla::SharedPtr<carla::sensor::SensorData> data) {
  PROF_READ_BEGIN("OUSTER");

  auto lidar_data =
      std::dynamic_pointer_cast<carla::sensor::data::LidarMeasurement>(data);
  if (!lidar_data)
    return;
  ...
  const uint8_t *raw_bytes =
      reinterpret_cast<const uint8_t *>(lidar_data->data());

  PROF_READ_T1();

  // Send via TCP Bridge
  TCPSender::getInstance().send(SensorType::OUSTER_LIDAR, timestamp, raw_bytes,
                                payload_size);
