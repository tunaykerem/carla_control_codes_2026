...
class World(object):
    ...
    def restart(self):
        ...
        print("\n=== Mevcut Araçlar ===")
        for i, bp in enumerate(blueprint_list, 1):
            print(f"{i}. {bp.id}")
        print("=" * 40 + "\n")
        ...
        while self.player is None:
            ...
            spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform() # spawn points değiştirme yeri
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            self.show_vehicle_telemetry = False
            self.modify_vehicle_physics(self.player)
        # Set up the sensors.
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0] == 'sensor.lidar.ray_cast':
            ...
            array = array[:, :, :3]
            array = array[:, :, ::-1]
            # Brighten image to match simulation - multiply by factor and clip
            #array = np.clip(array.astype(np.float32) * 3.5, 0, 255).astype(np.uint8)
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
...
def game_loop(args):
    ...
    try:
        ...
        # Buraya harita değişikliğini ekleyin:
...
def main():
    ...
    argparser.add_argument(
       ...
       '--filter', metavar='PATTERN', default='vehicle.ue4.audi.tt',
